# from https://github.com/Jonathan-Pearce/DDPG_PER
import numpy as np
import torch
...
class DDPG(object):
    ...
    def save_data(self):
        torch.save(self.actor.state_dict(), "data/actor.tar")
        torch.save(self.actor_target.state_dict(), "data/actor_target.tar")
        torch.save(self.critic.state_dict(), "data/critic.tar")
        torch.save(self.critic_target.state_dict(), "data/critic_target.tar")

    def load_data(self):
        self.actor.load_state_dict(torch.load("data/actor.tar"))
        self.actor_target.load_state_dict(torch.load("data/actor_target.tar"))
        self.critic.load_state_dict(torch.load("data/critic.tar"))
        self.critic_target.load_state_dict(torch.load("data/critic_target.tar"))
